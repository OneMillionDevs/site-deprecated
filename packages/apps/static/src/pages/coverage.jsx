  ...
  return (
    <Site>
      <A.Box color='white' gradient='blueDark' py={4} textCenter>
        <A.Flex between px={4}>
          <A.Heading lg heavy>News Coverage</A.Heading>
          <A.Button xs variant='green'>Add Coverage</A.Button>
        </A.Flex>
      </A.Box>
      <A.Container maxWidth={780} py={4}>
          {posts.filter(({node: post} )=> post.frontmatter.template === 'coverage' ).map(({ node: post }) => (
            <>
